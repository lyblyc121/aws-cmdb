...
def get_key_file(ssh_key):
    ...
    file_path = '/tmp/cmdb_cmdb_id_rsa'
...
#获取size转化到xlarge的因子数
def get_factor_num_with_xlarge(size):
    """
        获取size转化到xlarge的因子数。
    :param size:
    :return:
...
def catch_exception(fun):
    def _catch_exception(*args, **kwargs):
        try:
            ...
            ins_log.write_log('error', str(e))
            ins_log.write_log('error', traceback.format_exc())

    return _catch_exception

#获取实例的family，size，size转化到xlarge的因子数
def get_instance_type_info(platform, instance_type):
    family, size = instance_type.split(".")[0:2]
    if platform != "Linux/UNIX":
        return family, size, 1
    return family, "xlarge", get_factor_num_with_xlarge(size)
