...
def add_host():
    add_user_url = 'https://cmdb.domain.com/api/cmdb2/v1/cmdb/server/'
    csrf_task_url = 'https://cmdb.domain.com/api/task/v2/task/accept/'
    ...
    if req_code != 0:
        logging.info(json.loads(req.text)['msg'])
        exit(-111)
    else:
        logging.info(json.loads(req.text)['msg'])
