...
def collect():
    ...
    data['cpu'] = exec_shell("cat /proc/cpuinfo | grep 'cpu cores' | uniq | awk {'logging.info $4'}") + 'Core'  # CPU核心数
    data['memory'] = get_memory()
    data['disk'] = exec_shell("df --total -h | grep total | awk {'logging.info $2'}")  # 总空间
    data['disk_utilization'] = exec_shell("df --total -h | grep total | awk {'logging.info $5'}")  # 总使用百分比
...
def get_memory():
    '''获取内存信息'''
    distributor = exec_shell("free | grep Mem | grep -v total  | awk {'logging.info $2'}")
