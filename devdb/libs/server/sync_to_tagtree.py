...
from settings import CMDB_TASK_DB_INFO
...
class SyncTagTree():
    def __init__(self, mb):
        self.mb = mb
        self.server_table = 'asset_server'  # cmdb-task tagtree server表名字，默认无需更改
        self.server_tag_table = 'asset_server_tag'  # cmdb-tasks'tag tree server和标签的关联表
    ...
    def server_sync_task(self):
        """
        server信息同步到cmdb-task里面的tag tree数据库里面
        :return:
        """
...
def main():
    """
    检查用户是否配置了同步cmdb-task MySQL信息,
    如果检测到配置，初始化MySQL，同步数据
    拿不到ORM直接使用原生SQL语句操作
    :return:
    """
    host = CMDB_TASK_DB_INFO.get('host')
    port = CMDB_TASK_DB_INFO.get('port')
    user = CMDB_TASK_DB_INFO.get('user')
    passwd = CMDB_TASK_DB_INFO.get('passwd')
    db = CMDB_TASK_DB_INFO.get('db')

    if not host or not port or not user or not passwd or not db:
        print('[Error]: Not fount CMDB_TASK_DB_INFO, auto pass...')
        return False

    try:
        mb = MysqlBase(**CMDB_TASK_DB_INFO)
        obj = SyncTagTree(mb)
        obj.server_sync_task()

    except Exception as e:
        msg = '[Error]: 请确认下CMDB_TASK 数据库配置信息是否正确'
