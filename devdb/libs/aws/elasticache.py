...
from models.db import ElastiCache as DB
...
class CacheApi():
    ...
    def format_region_memcached(self, memcached_data):
        ...
        asset_data['db_host'] = memcached_data.get('CacheNodes')[0].get("Endpoint").get('Address')
        asset_data['db_port'] = memcached_data.get('CacheNodes')[0].get("Endpoint").get('Port')
        ...
        asset_data['db_node_num'] = str(memcached_data.get('NumCacheNodes'))  # 节点个数
        asset_data['db_shard_num'] = "未配置"  # 分片数量
        asset_data['state'] = memcached_data.get('CacheClusterStatus')  # 状态
        asset_data['cluster_enabled'] = memcached_data.get('ClusterEnabled',"0")  # 是否开启cluster
    ...
    def format_region_redis(self, redis_data):
        ...
        if  redis_data.get('ConfigurationEndpoint'):
            asset_data['db_host'] = redis_data.get('ConfigurationEndpoint').get('Address')
            asset_data['db_port'] = redis_data.get('ConfigurationEndpoint').get('Port')
        else:
            asset_data['db_host'] =redis_data.get('NodeGroups')[0].get("PrimaryEndpoint").get("Address")
            asset_data['db_port'] = redis_data.get('NodeGroups')[0].get("PrimaryEndpoint").get("Port")
        ...
        asset_data['db_node_num'] = str(len(redis_data.get('MemberClusters')))  # 节点个数
        asset_data['db_shard_num'] = str(len(redis_data.get('NodeGroups')))  # 分片数量
        asset_data['db_detail'] = ','.join(redis_data.get('MemberClusters'))  # 把子节点机器名字写到备注里面吧
        asset_data['cluster_enabled'] = redis_data.get('ClusterEnabled',"0")  # 是否开启cluster
        return asset_data
            ...
            for i in cache_list:
                ...
                if exist_redis:
                    session.query(DB).filter(DB.db_code == db_code).update({
                        ...
                        DB.state: i.get('state'), DB.db_node_num: i.get('db_node_num'),
                        DB.db_shard_num: i.get('db_shard_num'),DB.cluster_enabled: i.get('cluster_enabled')})
                else:
                    new_db = DB(idc=self.idc, db_code=db_code, db_class=i.get('db_class'),
                                db_host=i.get('db_host'), db_port=i.get('db_port'),
                                ...
                                db_node_num=i.get('db_node_num'), db_shard_num=i.get('db_shard_num'),
                                cluster_enabled = i.get('cluster_enabled')
                                )
                    session.add(new_db)
            session.commit()
