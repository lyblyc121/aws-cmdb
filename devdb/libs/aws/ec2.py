...
class Ec2Api():
    ...
    def get_volumes(self,id):
        ...
        response = self.client.describe_volumes(VolumeIds=[id])
        size =  int()
        for voleme in response.get("Volumes"):
            size += int(voleme.get("Size"))
        return size
        ...
        if ret:
            for r in ret:
                for i in r['Instances']:
                    ...
                    try:
                        tag_list = i.get('Tags')
                        tag_list = filter(lambda x: x["Key"] == "project", tag_list)
                        asset_data['project'] = list(tag_list)[0].get('Value')

                    except (KeyError, TypeError, IndexError):
                        asset_data['project'] = i.get('InstanceId', 'Null')  # 拿不到hostnameg给instance_id
                    asset_data['tags'] = str()
                    for tag in i.get("Tags"):
                        asset_data['tags'] = asset_data['tags']+ "//"+ tag.get("Key")+":"+tag.get("Value")
                    asset_data['volumes_total'] = int()
                    for j in i["BlockDeviceMappings"]:
                        asset_data['volumes_total'] += self.get_volumes(j["Ebs"]['VolumeId'])
    ...
    def sync_cmdb(self):
        ...
        with DBContext('w') as session:
            for server in server_list:
                ...
                project = server.get('project', 'Null')
                volumes_total = server.get('volumes_total', 'Null')
                tags = server.get('tags', 'Null')
                ...
                if exist_hostname:
                    session.query(Server).filter(Server.hostname == hostname).update(
                        {Server.ip: ip, Server.public_ip: ip, Server.private_ip: private_ip, Server.idc: 'AWS',
                         Server.region: region,Server.project:project,Server.volumes_total:volumes_total,Server.tags:tags})

                else:
                    new_server = Server(ip=ip, public_ip=ip, private_ip=private_ip, hostname=hostname, port=22,
                                        idc=self.account,
                                        region=region,
                                        state=self.state, admin_user=self.default_admin_user,instance_id=instance_id,
                                        project=project,volumes_total=volumes_total,tags=tags)
