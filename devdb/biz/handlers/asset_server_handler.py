...
class GetsHostProjectHandler(BaseHandler):
    def get(self, *args, **kwargs):
        project_list = []
        with DBContext('r') as session:
...
class ServerHostHandler(BaseHandler):
    def get(self, *args, **kwargs):
        key = self.get_argument('key', default=None, strip=True)
        ...
        with DBContext('r') as session:
            if project:
                if key:
                    project_data = session.query(Server).filter(Server.project==project).filter(
                        or_(Server.hostname.like('%{}%'.format(key)),
                            Server.ip.like('%{}%'.format(key)),
                            Server.public_ip.like('%{}%'.format(key)),
                            Server.private_ip.like('%{}%'.format(key)),
                            Server.port.like('%{}%'.format(key)),
                            Server.idc.like('%{}%'.format(key)),
                            Server.project.like('%{}%'.format(key)),
                            Server.admin_user.like('%{}%'.format(key)),
                            )
                    ).all()
                else:
                    project_data = session.query(Server).filter(Server.project == project).all()
            else:
                if key:
                    project_data = session.query(Server).filter(
                        or_(Server.hostname.like('%{}%'.format(key)),
                            Server.ip.like('%{}%'.format(key)),
                            Server.public_ip.like('%{}%'.format(key)),
                            Server.private_ip.like('%{}%'.format(key)),
                    ...
                    project_data = session.query(Server).all()
            server_list = []
            for msg in project_data:
                ...
                for t in server_tags:
                    tag_list.append(t[0])
                status = session.query(ServerDetail).filter(
                    ServerDetail.instance_id == data_dict["instance_id"]).first()
                status = model_to_dict(status)
                data_dict['status'] = status["instance_state"]
                data_dict['create_time'] = str(data_dict['create_time'])
                data_dict['update_time'] = str(data_dict['update_time'])
                data_dict['tag_list'] = tag_list
                server_list.append(data_dict)
            server_list_re = server_list[(pageNum - 1) * pageSize:pageNum * pageSize]
        self.write(dict(code=0, msg='获取成功', count=len(server_list), data=server_list_re))
...
asset_server_urls = [
    ...
    (r"/v1/cmdb/server/project/", GetsHostProjectHandler),
    (r"/v1/cmdb/server2/", ServerHostHandler),
]
