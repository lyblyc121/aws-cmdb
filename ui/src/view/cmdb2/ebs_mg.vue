...
export default {
      ...
      columns: [
            ...
            return h('div', [
                h('Icon', 
                {
                    props: {
                        ...
                        size: '12px',
                        color: params.row.State === 'available' ? 'blue' : 'green',
                    }
                }),
                h('a',params.row.State)
            ])
          }
        ...
        },
        {
            ...
            render: (h, params) => {
                return h(
                "a",
                params.row.Encrypted === 'true' ? '已加密' : '未加密'
                );
            }
        },
        {
  ...
  methods: {
    ...
    // 获取主机信息
    getEBSList(value) {
      getEBSList(this.pageNum, this.pageSize, value).then(res => {
        if (res.data.code === 0) {
    ...
    handleSearch() {
      this.getEBSList(this.searchValue);
    },
    // 点击节点
