      ...
      <div class="search-con search-con-top">
        <Input
            ...
            <Button @click="handleSearch" class="search-btn" type="primary">搜索</Button>
             <Button type="primary" class="search-btn" @click="exportData(1)">
              <Icon type="ios-download-outline"></Icon>导出数据
            </Button>
            <Button type="success" class="search-btn" @click="handleSync">手动刷新</Button>
...
export default {
    ...
    return {
      ...
      columns: [
        ...
        
        {
          ...
          render: (h, params) => {
            // let tmpSecurity = ""
            // if (params.row.security_group !== "") {
            //   let tmp = params.row.security_group.toString()
            //   let groups = tmp.slice(1, -1)
            //   groups = groups.split('{')
            //   groups.splice(0,1)
            //   let securityGroups = []
            //   groups.forEach(item=> {
            //     let tmp = item.split(',')[0].split(':')[1]
            //     tmp = tmp.slice(tmp.indexOf("'")+1,-1)
            //     securityGroups.push(tmp)
            //     tmpSecurity = tmpSecurity + tmp + '\n'
            //   })
            //   params.row.security_groups = securityGroups
            // }
            return h(
              "a",
              params.row.security_group
            );
          }
        },
        {
          ...
          align: "center",
          render: (h, params) => {
            let saftety = params.row.security_state === "" ? false : true
            // console.log(params.row.security_state)
            
            // if (params.row.security_state === "" || params.row.security_state === "1") {
            //   saftety = true
            // } else { saftety = false }
            //  = params.row.security_state === "" ? true : false
            // params.row.security_state = saftety
            // console.log(saftety)

             return h("div", [
              h("Checkbox", { props: { value: saftety }}, "")
            ])
          } 
        },
        {
          ...
          render: (h,params) => {
            return h('a', 
            // {
            //   props: { placeholder: params.row.server_mark},
            //   on: {
            //     'on-change': (e) => {
            //       params.row.server_mark = e.currentTarget.value
            //     }
            //   },
            // }
            params.row.server_mark
            )
          ...
          render: (h,params) => {
            return h(
                "Button",
                {
                  ...
                  on: {
                    click: () => {
                      this.handleHost(params.row, '更新主机信息')
                    }
                  }
                },
                "修改"
  ...
  methods: {
    ...
    exportData(type) {
      if (type === 1) {
        let exData = []
        getRiskyServerList(1, this.pageTotal).then(res => {
          if (res.data.code === 0) {
            exData = res.data.data
            for(let i=0; i<exData.length; i++) {
              exData[i].security_group = this.listJsonTrans(exData[i].security_group)
              exData[i].risk_port = exData[i].risk_port.replace(/,/g, ';')
              exData[i].security_state = exData[i].security_state === "" ? "no" : "yes"
            }
            console.log(exData)
            this.$refs.selection.exportCsv({
              filename: "cmdb_risky_server_data",
    ...
    listJsonTrans(str) {
      ...
      return tmpSecurity.substring(0,tmpSecurity.length-2)
    },
    ...
    // 获取主机信息
    getServerList(value) {
      getRiskyServerList(this.pageNum, this.pageSize, value).then(res => {
        if (res.data.code === 0) {
          this.pageTotal = res.data.count;
          this.tableData = res.data.data;
          console.log(res.data)
          for(let i=0; i<this.tableData.length; i++) {
            this.tableData[i].security_group = this.listJsonTrans(this.tableData[i].security_group)
          }
    ...
    // },
    handleSync() {
      // this.getRiskyServerList(this.searchValue)
      syncAWShost().then(res => {
        if (res.data.code === 0) {
          this.$Message.success(`${res.data.msg}`)
          this.getServerList()
          console.log("同步成功")
    ...
    handleDetail(paramsRow) {
      ...
      this.detailData = {
        ...
        security_group: paramsRow.security_group,
        risk_port: riskPorts,
        server_mark: paramsRow.server_mark
      }
    },
